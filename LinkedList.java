...
public class LinkedList<E extends Comparable<E>> {
	...
	private void remove(E el, Node reference) {
		if (reference != null) {
			if ((E) first.data == el) {
				first = first.next;
				reference = first;
				remove(el, reference);
			} else if ((E) first.next.data == el) {
	...
	public void duplicateAll(E el) {
		...
		// with first as an argument. It must be recursive, no loop allowed.
		Node reference = first;
		duplicate(el, reference);
	}

	private void duplicate(E el, Node reference) {
		if (reference != null) {
			if ((E) reference.data == el) {
				reference.next = new Node(el, reference.next);
